...
final class KeyConverterNodeBuilder implements NodeBuilder
{
    ...
    /** @var WeakMap<ShapedArrayType|ShapedListType, array<string, string>> */
     ...
     * @param array<string, string> $attributeSources element key => source key
     * @return array{
     *     0: array<mixed>,
    ...
    /**
     * @return array<string, string> element key => source key
     */
    private function resolveAttributeSources(ShapedArrayType|ShapedListType $type): array
    {
        ...
        foreach ($type->elements as $key => $element) {
            ...
            if (isset($claimedBy[$sourceKey])) {
                throw new SeveralAttributesMapToSameKey($sourceKey, $claimedBy[$sourceKey], (string)$key);
            }

            $claimedBy[$sourceKey] = (string)$key;
            $sources[(string)$key] = $sourceKey;
        }
