...
function castTo(Type $type, Node $node): Node
{
    return CastNode::to($type, $node);
}
 ...
 */
function forEach_(Node $value, string $key, string $item, Node|array $body): ForEachNode
{
...
function phpFile(Node ...$nodes): PhpFileNode
{
    return new PhpFileNode(...$nodes);
}
...
function try_(Node ...$body): TryNode
{
